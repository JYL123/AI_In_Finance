...
jasmine_util_1.describeWithFlags('convertToTensor', test_util_1.ALL_ENVS, function () {
    it('primitive number', function () {
        var a = tensor_util_1.convertToTensor(3, 'a', 'test');
        expect(a.rank).toBe(0);
        expect(a.dtype).toBe('float32');
        test_util_1.expectNumbersClose(a.get(), 3);
    });
    it('primitive boolean, parsed as float', function () {
        var a = tensor_util_1.convertToTensor(true, 'a', 'test');
        expect(a.rank).toBe(0);
        expect(a.dtype).toBe('float32');
        test_util_1.expectNumbersClose(a.get(), 1);
    });
    it('primitive boolean, parsed as bool', function () {
        var a = tensor_util_1.convertToTensor(true, 'a', 'test', 'bool');
        expect(a.rank).toBe(0);
        expect(a.dtype).toBe('bool');
        expect(a.get()).toBe(1);
    });
    it('array1d', function () {
        var a = tensor_util_1.convertToTensor([1, 2, 3], 'a', 'test');
    ...
    it('array3d', function () {
        var a = tensor_util_1.convertToTensor([[[1], [2]], [[3], [4]]], 'a', 'test');
    ...
    it('passing a tensor returns the tensor itself', function () {
        var s = tf.scalar(3);
        var res = tensor_util_1.convertToTensor(s, 'a', 'test');
        expect(res).toBe(s);
    });
    it('passing a tensor with casting returns the tensor itself', function () {
        var s = tf.scalar(3);
        var res = tensor_util_1.convertToTensor(s, 'a', 'test', 'bool');
        expect(res).toBe(s);
    });
    it('fails to convert a dict to tensor', function () {
        expect(function () { return tensor_util_1.convertToTensor({}, 'a', 'test'); })
    ...
    it('fails to convert a string to tensor', function () {
        expect(function () { return tensor_util_1.convertToTensor('asdf', 'a', 'test'); })
            .toThrowError('Argument \'a\' passed to \'test\' must be a Tensor ' +
            'or TensorLike, but got String');
    });
    it('fails to convert a non-valid shape array to tensor', function () {
        var a = [[1, 2], [3], [4, 5, 6]];
        expect(function () { return tensor_util_1.convertToTensor(a, 'a', 'test'); })
